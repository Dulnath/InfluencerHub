  ...
  const loadUsersData = async (
    ...
    switch (optType) {
      case "search":
        ...
        return await axios
          .get(
            `http://localhost:8080/api/users/search/${value}`
      ...
      case "sort":
        ...
        return await axios
          .get(
            `http://localhost:8080/api/users/getUsers?_sort=${filterOrSortValue}&_order=asc&_start=${start}&_end=${end}`
          )
          .then((response) => {
            setData(response.data);
            setCurrentPage(currentPage + increase);
      ...
      case "filter":
        ...
        return await axios
          .get(
            `http://localhost:8080/api/users/getUsers?status=${filterOrSortValue}&_order=asc&_start=${start}&_end=${end}`
          )
          .then((response) => {
            setData(response.data);
            setCurrentPage(currentPage + increase);
      ...
      default:
        return await axios
          .get(
            `http://localhost:8080/api/users/getUsers?_start=${start}&_end=${end}`
          )
          .then((response) => {
            setData(response.data);
            setCurrentPage(currentPage + increase);
  ...
  return (
    <MDBContainer>
      ...
      <div style={{ marginTop: "100px" }}>
        <h2 className="text-center">Search, Filter, Sort Users</h2>
        <MDBRow>
          <MDBCol size="12">
            <MDBTable>
              <MDBTableHead dark>
        ...
        >
          {renderPagination(sortFilterValue)}
        </div>
      </div>
      {data.length > 0 && (
        <MDBRow>
